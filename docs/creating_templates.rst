...
Under the hood, footing uses `cookiecutter <https://cookiecutter.readthedocs.io/en/latest/>`_ to gather user
...
Once you have created a cookiecutter template and published it, it will work with footing out of the box, but there are
...
the cookiecutter template is only used by footing and not by ``cookiecutter`` or another templating library:
